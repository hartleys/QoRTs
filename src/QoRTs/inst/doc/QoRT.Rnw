...
\noindent or via novosort, which may be faster and more memory-efficient:
\begin{verbatim}
...
\noindent \emph{Required Fields}:
  \begin{itemize}
    \item \emph{lanebam.ID}: The ID of the bam file. Must be unique.
    \item \emph{lane.ID}: The ID of the lane or batch for the bam file.
    \item \emph{group.ID}: The ID of the "group". For example: "Case" or "Control".
    \item \emph{sample.ID}: The ID of the biological sample for the bam file. Each sample can have multiple bam-files associated with it.
    \item \emph{qc.data.dir}: The directory in which the java utility should save all the output data.
  \end{itemize}

\noindent \emph{Optional Fields}: The following additional fields are optional. Note that you should not use these names 
  \begin{itemize}
    \item \emph{input.read.pair.count}: The number of reads in the original fastq file, before alignment.
    \item \emph{multi.mapped.read.pair.count}: The number of reads that were multi-mapped by the aligner.
    \item \emph{cycle.CT}: The number of bases in each read. By default this will be auto-detected from the data and need not be included.
  \end{itemize}
...
\emph{Stranded data:} By default, QoRTs assumes that the data is NOT strand-specific. For strand-specific data, the \verb|--stranded| flag must be added to the command line, like so:
...
All plotting functions in QoRTs require a \Robject{QoRT\_Plotter} object. A \Robject{QoRT\_Plotter} is a \Robject{RefClass} object that contains all the QC data along with a set of parameters that determine how to color and draw each bam file's data. A full accounting of all possible options available in the is beyond the scope of this manual, but can be found in the help docs for the \Robject{QoRT\_Plotter} class.
...
This plot includes many sub-plots, all in a single frame. The sub-plots are:
  \begin{itemize}
    ...
    \item \emph{(x)}: Strandedness test. Described in section \ref{subsec:plotDesc-15}
    \item \emph{(y)}: Read-mapping statistics. Described in section \ref{subsec:plotDesc-17}
    \item \emph{(z)}: Chromosome counts. Described in section \ref{subsec:plotDesc-18}
    \item \emph{(aa)}: Comparison of normalization factors. Described in section \ref{subsec:plotDesc-19}
    \item \emph{(ab)}: Leading-clipped nucleotide rates. Described in section \ref{subsec:plotDesc-11}
    \item \emph{(ac)}: Trailing-clipped nucleotide rates. Described in section \ref{subsec:plotDesc-11b}
    \item \emph{(ad)}: Raw nucleotide rate by read position. Described in section \ref{subsec:plotDesc-10}
    \item \emph{(ae)}: Aligned nucleotide rate by read position. Described in section \ref{subsec:plotDesc-10b}
  \end{itemize}

\pagebreak
%----------
\subsubsection{Colored by Lane/Batch} \label{subsec:plotterDesc2}
%----------
...
The example plots in the following section all use the \verb|byLane.plotter| object (from Section \ref{subsec:plotterDesc2}), which colors each bamfile by its lane ID.
...
For each bam-file, Figure \ref{indivPlot-12-fig} this plot displays the rate (y-axis) for which the bam-file's read-pairs are assigned to the given categories.
...
\pagebreak
%----------
\subsubsection{Splice Junction Event Rates} \label{subsec:plotDesc-14}
%----------

\myincfig{indivPlot-14}{0.65\textwidth}{Splice junction events}

For each bam-file, Figure \ref{indivPlot-14-fig} displays the rate (y-axis) at which each type of splice junction appear in the bam-file's reads. Splice junctions are split into 4 groups, first by whether the splice junction appears in the transcript annotation gtf ("known" vs "novel"), and then by whether the splice junction has 4 or more reads covering it, or 1-3 reads ("Hi" vs "Lo").
...
\myincfig{indivPlot-19}{0.65\textwidth}{Mapping stats}
...
%-----------------------------------------------------------
\section{Other Minor Utilities} \label{sec:minorUtils}
%-----------------------------------------------------------
...
%----------
\subsection{Optional: Generating a flattened annotation file} \label{sec:flatAnnoGen}
%----------
...
\emph{DEXSeq:} DEXSeq also requires a flattened annotation file, which is formatted similarly. In order to produce a flattened gff file that DEXSeq can read, add the \verb|--DEXSeqFmt| option.

This gtf file conforms to the UCSC gff file definition, (found here: \url{http://genome.ucsc.edu/FAQ/FAQformat.html}). It will contain 4 different feature types (column 3): "aggregate\_gene", "exonic\_part", "splice\_site", and "novel\_splice\_site". This file is an expanded variant of the flattened gff files produced by DEXSeq.

  \begin{itemize}
    \item \emph{aggregate\_gene} Aggregate genes are usually single genes. However, whenever genes overlap, the set of overlapping genes are combined into a single aggregate gene.
    \item \emph{exonic\_part}
    \item \emph{splice\_site}
    \item \emph{novel\_splice\_site}
  \end{itemize}
...
\noindent Common options and flags for this function include:
  \begin{itemize}
    \item \emph{--sizefactor 1.0}: A float value. All the coverage values will be divided by this factor. Useful for comparing two samples that may have different normalization factors.
    \item \emph{--stranded}: Flag to indicate that data should be treated as stranded.
    \item \emph{--stranded\_fr\_secondstrand}: Flag to indicate that the data is of the fr\_secondstrand stranded library type. (See section \ref{sec:initialProcessing} for more information on the two stranded library types) 
    \item \emph{--negativeReverseStrand}: If this flag is set, then the negative strand will be counted in negative numbers. This can be useful for plotting both strands in a single multiwig track, via a trackhub. (see \url{http://genome.ucsc.edu/goldenPath/help/trackDb/trackDbDoc.html})
  \end{itemize}
...
\noindent Common options and flags for this function include:
  \begin{itemize}
    \item \emph{--calcMean}: If this flag is raised, the utility will calculate the average rather than the total coverage for each window.
    \item \emph{--ignoreSizeFactors}: By default the utility attempts to auto-detect the presence of normalization factors in the filelist.txt file. If this flag is raised, the utility will ignore all but the first column of filelist.txt, and set all size factors to 1.
    \item \emph{--makeNegative}: If this flag is raised, the output will be multiplied by -1.
  \end{itemize}
