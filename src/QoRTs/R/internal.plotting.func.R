...
makePlot.generic <- function(data.list, plotter, x.name, y.name, norm.x, avg.y, plot.type = "lines", xlim = NULL, ylim = NULL, plot.means = FALSE, plot.medians = FALSE, vert.offset = 0, pre.plot.func = NULL, ...){
...
makePlot.generic.pair <- function(data.list.r1, data.list.r2, plotter, x.name, y.name, norm.x, avg.y, xlim , ylim = NULL, vert.offset = 0, horiz.offset = 0, draw.horiz.lines = FALSE, plot.type = "lines", r2.buffer = 10, override.lty = -1, x.is.log = FALSE, y.is.log = FALSE, pre.plot.func = NULL, y.axis.las = 1, ...){
  ...
  if(x.is.log) pretty.axis.x.labels <- 10 ^ pretty.axis.x;
  ...
  if(draw.horiz.lines){
    ...
    if(y.is.log) pretty.axis.y.labels <- 10^ pretty.axis.y ;
...
makePlot.generic.NVC.pair <- function(data.list.r1, data.list.r2, plotter, x.name, y.name, xlim, ylim, r2.buffer = 10, 
...
makePlot.generic.points <- function(tf.list, plotter, plot.type = "points", ylim = NULL, leave.blank.cols = 0, label.y = TRUE, cex.x.axis = NULL, pre.plot.func = NULL, draw.lines = FALSE, ...){
  priorities <- sort(unique(plotter$lanebam.params$plot.priority));
...
makePlot.generic.points.right <- function(tf.list, plotter, plot.type = "points", section.offset = 0, ylim.plot = NULL, ylim.data = NULL, label.y = TRUE, cex.x.axis = NULL, ...){
  priorities <- sort(unique(plotter$lanebam.params$plot.priority));
...
internal.plot.legend <- function(plotter, legend.type, legend.pos, ...){
  ...
  } else if(plotter$plot.type == "highlightBySample.colorByLane.NVC") {
...
internal.get.main.title.fragment <- function(plotter, plot.type){
  if(plotter$plot.type == "highlightBySample.colorByLane" & plot.type == "NVC")        return( paste0("\nWith Sample ",plotter$title.highlight.name," in Bold"));
  if(plotter$plot.type == "highlightBySample.colorByLane")        return( paste0("\nWith Sample ",plotter$title.highlight.name," colored by lane"));
  if(plotter$plot.type == "highlightBySample")                    return( paste0("\nWith Sample ",plotter$title.highlight.name," in ", plotter$legend.params$lines.col[1]));
  if(plotter$plot.type == "highlightBySample.colorByLane.NVC")    return( paste0("\nWith Sample ",plotter$title.highlight.name," in Bold"));
