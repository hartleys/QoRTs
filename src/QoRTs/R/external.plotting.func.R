...
makePlot.qual.pair <- function(plotter, y.name, r2.buffer = NULL, debugMode = DEFAULTDEBUGMODE, ...) {
  ...
  if(is.null(plotter$res@qc.data[["quals.r1"]]) ){
   ...
   makePlot.generic.pair(plotter$res@qc.data[["quals.r1"]], 
...
makePlot.cigarOp.byCycle <- function(plotter,op, r2.buffer = NULL, rate.per.million = TRUE, debugMode = DEFAULTDEBUGMODE,...){
  ...
  cigarOpRate.list.r2 <- get.cigarOpRate.from.dl(plotter$res@qc.data[["cigarOpDistribution.byReadCycle.R2"]]);
   ...
   makePlot.generic.pair(cigarOpRate.list.r1, 
                             cigarOpRate.list.r2, 
                             plotter = plotter, 
                             x.name = "CYCLE",
                             y.name = "cigarRate",
...
makePlot.insert.size <- function(plotter, calc.rate = TRUE, pct.cutoff = 0.98, plot.medians = TRUE, plot.means = NULL, debugMode = DEFAULTDEBUGMODE,...){
  ...
  makePlot.generic(res@qc.data[["insert.size"]], plotter, 
...
makePlot.genebody.coverage <- function(plotter, plot.medians = NULL, plot.means = TRUE, debugMode = DEFAULTDEBUGMODE, ... ){
  ...
  makePlot.generic(data.list, plotter, 
...
makePlot.missingness.rate <- function(plotter,  r2.buffer = NULL, debugMode = DEFAULTDEBUGMODE, ...){
  ...
  data.list.r2 <- lapply(raw.data.list.r2,function(df){
   ...
   makePlot.generic.pair(        data.list.r1, 
                             data.list.r2, 
                             plotter = plotter, 
                             x.name = "readPos",
                             y.name = "RATE",
...
makePlot.cigarLength.distribution <- function(plotter,op, r2.buffer = NULL, perMillion = TRUE, log.x = FALSE, log.y = FALSE, debugMode = DEFAULTDEBUGMODE,...){
  ...
  get.cigarLenData.from.dl <- function(dl){
   ...
   makePlot.generic.pair(        cigarLenData.list.r1, 
...
makePlot.raw.NVC <- function(plotter,  r2.buffer = NULL,  points.highlighted = TRUE,
  ...
  if(is.null(plotter$res@qc.data[["NVC.raw.R1"]]) ){
   ...
   makePlot.generic.NVC.pair(    plotter$res@qc.data[["NVC.raw.R1"]], 
...
makePlot.minus.clipping.NVC <- function(plotter,  r2.buffer = NULL, points.highlighted = TRUE,
  ...
  if(is.null(plotter$res@qc.data[["NVC.minus.clipping.R1"]]) ){
   ...
   ylim <- c(0.1,0.55);
   makePlot.generic.NVC.pair(    plotter$res@qc.data[["NVC.minus.clipping.R1"]], 
...
makePlot.NVC.lead.clip <- function(plotter, clip.amt = 10,  r2.buffer = clip.amt / 10, points.highlighted = TRUE,
  ...
  if(is.null(plotter$res@qc.data[["NVC.lead.clip.R1"]]) ){
   ...
   makePlot.generic.NVC.pair(    data.list.r1, 
                             data.list.r2, 
                             plotter = plotter, 
                             x.name = "readPos",
                             y.name = "CT",
...
makePlot.strandedness.test <- function(plotter, plot.target.boxes = FALSE, debugMode = DEFAULTDEBUGMODE, ...){
  ...
  tf.list <- generic.points.tf(plotter$res@qc.data[["summary"]], 
                               ...
                               offset.horiz = 0.5,
                               horiz.offsets = plotter$lanebam.params$horiz.offsets);
  makePlot.generic.points(tf.list,plotter,ylim=c(0,1),pre.plot.func=pre.plot.func,...)
  title(ylab="Rate");
...
makePlot.dropped.rates <- function(plotter, debugMode = DEFAULTDEBUGMODE,...){
  ...
  tf.list <- generic.points.tf(plotter$res@qc.data[["summary"]], 
                               ...
                               offset.horiz = 0.5,
                               horiz.offsets = plotter$lanebam.params$horiz.offsets);
  makePlot.generic.points(tf.list,plotter,...)
  title(ylab="Rate");
...
makePlot.mapping.rates <- function(plotter, plot.mm = TRUE, y.counts.in.millions = TRUE, debugMode = DEFAULTDEBUGMODE,...){
  ...
  if(is.null(plotter$res@calc.data[["map.rates"]])){
    ...
    if(is.null(plotter$res@calc.data[["map.rates"]]$multi.mapped.reads) & plot.mm){
       ...
       makePlot.generic.points(tf.list,plotter, leave.blank.cols = 4, label.y = F,...);
       makePlot.generic.points.right(tf.list.2, plotter, section.offset = 4.5, ylim.data = c(0,1));
       usr <- par("usr");
       yrange <- usr[4] - usr[3];
       ...
       makePlot.generic.points(tf.list,plotter, leave.blank.cols = 3, label.y = F,...);
       makePlot.generic.points.right(tf.list.2, plotter, section.offset = 3.5, ylim.data = c(0,1));
       usr <- par("usr");
       yrange <- usr[4] - usr[3];
       rect.y <- c(usr[3] - yrange, usr[4] + yrange);
...
makePlot.norm.factors.vs.TC <- function(plotter, by.sample = TRUE, return.table = FALSE, debugMode = DEFAULTDEBUGMODE, ...){
  ...
  if( is.null(plotter$res@calc.data[["norm.factors.bySample"]]) ){
    ...
    makePlot.generic.points(tf.tf.list,plotter,pre.plot.func=pre.plot.func,...);
