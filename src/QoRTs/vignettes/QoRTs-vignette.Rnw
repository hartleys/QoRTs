...
\date{August 8 2014\\
April 14, 2015 %REPLACE_THIS_LINE_WITH_REVISION_DATE
\\
v0.2.17 %REPLACE_THIS_LINE_WITH_VERSION_NUMBER
}
...
There are a number of crucial points that require attention when using the QoRTs.jar QC command.
  ...
  \begin{itemize}
    ...
    \item \emph{Read Sorting:} For paired-end data reads must be sorted. By default, QoRTs can accept files sorted by name OR by position.
