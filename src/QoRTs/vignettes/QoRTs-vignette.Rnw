...
\date{August 8 2014\\
February 10, 2015 %REPLACE_THIS_LINE_WITH_REVISION_DATE
\\
v0.1.6 %REPLACE_THIS_LINE_WITH_VERSION_NUMBER
}
...
This vignette primarily covers the quality control functionality of QoRTs, and only briefly covers the other functions and capabilities. A comprehensive walkthrough covering the entire process from post-alignment all the way to differential expression analysis, along with a full example dataset and example output, can be found online: \url{ftp://nhgriftp.nhgri.nih.gov/pub/outgoing/mullikin/example_data/QoRTsFullExampleData.zip}.
