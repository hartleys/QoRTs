...
\date{August 8 2014\\
February 05, 2015 %REPLACE_THIS_LINE_WITH_REVISION_DATE
\\
v0.1.3 %REPLACE_THIS_LINE_WITH_VERSION_NUMBER
}
...
This vignette primarily covers the quality control functionality of QoRTs, and only briefly covers the other functionality. A comprehensive walkthrough covering the entire process from post-alignment all the way to differential expression analysis, along with a full example dataset and example output, can be found linked on the QoRTs GitHub site (\url{https://github.com/hartleys/QoRTs}).
...
For each replicate, Figure \ref{indivPlot-17-fig} displays the mapping rates and counts. 
