...
\date{August 8 2014\\
Revised 12 December 2014 %REPLACE_THIS_LINE_WITH_REVISION_DATE
\\
v0.0.27 %REPLACE_THIS_LINE_WITH_VERSION_NUMBER
}
...
Several QoRTs functions will require a "decoder" file, which describes each sample and all of its technical replicates (if any). 

\noindent \emph{Required Fields}:
  \begin{itemize}
    \item \emph{unique.ID}: A unique identifier for the row. QoRTs will also accept the synonym "lanebam.ID".
    \item \emph{lane.ID}: The ID of the lane or batch.
    \item \emph{group.ID}: The ID of the "group". For example: "Case" or "Control".
    \item \emph{sample.ID}: The ID of the biological sample from which the data originated. Each sample can have multiple rows, representing technical replicates (in which the same sample is sequenced on multiple lanes or runs).
...
\emph{IMPORTANT NOTE:} As mentioned in section \ref{sec:reqs}, input bam files MUST be sorted by \emph{NAME}. QoRTs can also be set to read bam files from a stdin, by simply substituting '-' (without quotes) for the bamfile name. See section \ref{sec:reqs} for more details.
...
makePlot.summary.basic(res, plot.device.name = "pdf");
@
The R PDF device uses vector drawings, however, some of the plots are too large to be efficiently stored as vectors. Thus, it is strongly recommended that the png R package be installed, as well as either the Cairo package or png device support. This will allow QoRTs to rasterize the plotting areas of certain large plots (in particular: the gene diversity plots and the various NVC plots). Unless these conditions are met the function above will issue a warning, and the resultant pdfs may be very large and difficult to print. Setting the \verb|rasterize.large.plots| parameter to \verb|FALSE| will deactivate these warnings. The \verb|raster.height| and \verb|raster.width| parameters can be used to increase the pixel resolution of the rasterized plots, if desired.
...
The \verb|paired| option can be used to calculate the GC-distribution for read-pairs rather than for all reads individually. This is disabled by default because it often results in a jagged distribution when a appreciable proportion of the reads have an insert size equal to or smaller than the read length. When this occurs, the read-pair will almost always have an even number of G/C nucleotides.
...
\noindent Common options and flags for this function include:
  \begin{itemize}
    ...
    \item \verb|--stranded_fr_secondstrand|: Flag to indicate that the data is of the fr\_secondstrand stranded library type. (See section \ref{sec:initialProcessing} for more information on the two stranded library types) 
