...
object makeOrphanJunctionBed {
  ...
  def run(sizeFactorFile : Option[String], sizeFactors : Option[List[Double]], 
    ...
    run_helper(title, infiles, outfile , stranded , sf, rgb , digits , includeFullSpliceNames , ! calcMean , nonflatgtf, Some("track name="+trackTitle+" description="+trackTitle+" "+rgbline+additionalTrackOptions), skipAnnotatedJunctions, skipNovelJunctions);
  }
  ...
  def run_helper(opt_title : Option[String], infile : Vector[String], outfile : String, 
    ...
    val allCounts : Vector[(Iterator[(String,Double,Vector[String])])] = lines.zip(sizeFactors).map(pair => {
      ...
      l.map(line => {
        val cells = line.split("	");
        (cells(0), string2double(cells(1)) / sf, cells.toVector);
      });
    });
    ...
    val counts : Iterator[(String,Double,Vector[String])] = allCounts.tail.foldLeft(allCounts.head)((soFar,curr) => {
      soFar.zip(curr).map(pair => {
        ...
        if(pair._1._3(0) != pair._2._3(0)) error("ERROR: junctionCount files do not match!");
        if(pair._1._3(2) != pair._2._3(2)) error("ERROR: junctionCount files do not match!");
        if(pair._1._3(3) != pair._2._3(3)) error("ERROR: junctionCount files do not match!");
        if(pair._1._3(4) != pair._2._3(4)) error("ERROR: junctionCount files do not match!");
        if(pair._1._3(5) != pair._2._3(5)) error("ERROR: junctionCount files do not match!");
        if(pair._1._3(6) != pair._2._3(6)) error("ERROR: junctionCount files do not match!");
    ...
    writeBed(title, counts, outfile , rgb, 
  ...
  def writeBed(title : String, 
    ...
    val lines : Vector[((String,Int),String)] = counts.filter{case(junctionName, ct, cells) =>{ 
      val featureCode = junctionName.split(":")(1).substring(0,1);
      (featureCode == "J" && (! skipAnnotatedJunctions)) || (featureCode == "N" && (! skipNovelJunctions));
    }}.map{case(junctionName, ct, cells) =>{
        
        val chrom = cells(2);
        val strand = cells(3);
        val start =  string2int(cells(4));
        val end = string2int(cells(5));
        ...
        val spliceName = if(includeFullSpliceNames) cells(6)+":"+cells(0) else cells(0);
